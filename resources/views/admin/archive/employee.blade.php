        ...
        <div class="row">
            ...
            @foreach($data as $info)
                <tr>
                    ...
                    <td>{{$info->address}}</td>
                    <td>
                        @if( $info->salary_type == 'salary')
                            <span class="bg-primary text-white">راتب </span>
                        @else
                            <span class="bg-danger text-white"> نسبة %</span>
                        @endif
                    </td>
                    <td>{{$info->balance}}</td>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-danger white">
                        <h4 class="modal-title white" id="delete">الاستعادة الموظف</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form action="{{route('admin.restoreEmployee','delete')}}" method="post">
                        @csrf
                        @method('post')
